%% Capability Map Generation for LRM 6 DOFs arm
clear all
close all
clc
...
% d
d = [126 0 136 0 179 0]; % link lengths
...
%% Random poses and relative capability
...
while counter < 10000000
    ...
    % forward kinematics of a random pose
    theta = 10*pi/9*rand([1,6]) - 10*pi/18;
    ...
    % capability value
    x = round(posgripper(1)/gridResolution) - gridmin(1)/gridResolution;
    y = round(posgripper(2)/gridResolution) - gridmin(2)/gridResolution;
    z = round(posgripper(3)/gridResolution) - gridmin(3)/gridResolution;
